...
The logical vector returns values of 0 and 1, which is the numeric version of this boolean.
The character vector returns NA values (not available) because letters or words cannot be converted to numbers.
Finally, the factor vector returns values of 1, 2, and 3, because R orders the factors numerically.
...
The Penguins data set contains `r ncol(penguins)` variables and `r nrow(penguins)` observations. Several are quite useful for consideration: `r names(penguins)[5]` has a mean value of `r round(mean(penguins$flipper_length_mm, na.rm = TRUE), digits = 2)` and a standard deviation of `r round(sd(penguins$flipper_length_mm, na.rm = TRUE), digits = 2)`. There are `r nlevels(pull(penguins, species))` species, including `r levels(pull(penguins, species))`. Finally, the years seen range from `r min(pull(penguins, year))` to `r max(pull(penguins, year))`.
